...
<section class="py-20 px-8 bg-white">
  <div class="container mx-auto max-w-4xl text-center">
    <h2 class="text-3xl font-bold text-gray-900 mb-6"><?= $page['main_title'] ?></h2>
    <p class="text-gray-700 text-justify leading-relaxed">
      <?= nl2br($page['main_content']) ?>
    </p>
    ...
    <div class="space-y-4 text-left">
      <?php foreach ($objectives as $i => $t): ?>
        <div class="flex items-center gap-4 bg-blue-800/40 border border-blue-500 rounded-xl p-4">
          ...
          <p class="text-blue-100 text-sm md:text-base"><?= $t['content'] ?></p>
        </div>
      <?php endforeach; ?>
    </div>
  </div>
...
<!-- Video Profile -->
<section class="py-20 px-8 bg-white">
  <div class="w-full aspect-video container mx-auto max-w-5xl text-center overflow-hidden"> <?php if(!empty($page['video_url'])): ?>
        <iframe class="w-full h-full" src="<?= $page['video_url'] ?>" frameborder="0" allowfullscreen></iframe>
    <?php else: ?>
        <span class="text-lg opacity-70">Belum ada video</span>
    <?php endif; ?>
  </div>
</section>
